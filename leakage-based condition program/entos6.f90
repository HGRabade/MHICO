...
!*** Versión Mhico3 con cambios (denotados por !***, las líneas comentadas así y sin explicaciones son líneas añadidas) respecto a Mhico
!Programa MEF válido para mallas de hasta 99999 elementos o 99999 nodos (25/02/2013). Autor: Héctor García Rábade
...
! aguas someras y subt) desconectado (pero no colgado) para acuíferos libres.  !*** "desconectado (pero no colgado)" en vez de "conectado"
...
! diferentes al haber el mismo número de nodos).  !*** Se usan espesores diferentes en el contorno móvil a los usados en Mhico
...
!3. Dependiente del ejemplo: Selección de dominio superficial a través de propiedad, modificación de hmin (abrir o cerrar las etiquetas !es). 
...
! H=altura de la lámina en mallainicial.txt, falta de CC en algún nodo del contorno,...). Parece que en el modelo conjunto no es posible prescindir de CC 
!de nivel para el flujo subterráneo en estacionario (al igual que no lo es el sup aislado estacionario). 
!*** Cambiada la última frase que había por la última frase que hay en el párrafo de arriba. Ahora no hay niveles en el contorno móvil.
...
! Ésta es adecuada si los elementos de la malla tienen forma regular (misma forma, pudiendo tener diferente tamaño) y funcionará mejor tanto más similar 
! sea la forma.
...
!14. Soluciones con error o no convergencia: Si se aplica el modelo superficial con un esquema semi-implícito utilizar un incremento de tiempo pequeño o 
! habrá errores en la solución (para valores suficientemente altos no habrá convergencia). Tampoco se recomienda usar un valor muy pequeño porque tardaría 
! demasiado (aún no se ha limitado el incremento de tiempo con el parámetro CFL).
!15. Si va a haber (al menos) una zona de flujo superficial aislado es necesario utilizar simulación transitoria. Tampoco se pueden utilizar iteraciones 
! previas de NS 2D en este caso.
!16. Si se utiliza la posibilidad comentada en !cñ en mallaaguassomeras se produce no convergencia (comentado allí lo que hacer para usarla).
!17. Si se utilizan iteraciones previas de NS 2D habrá que seleccionar una nu con la que se tenga un gradiente parecido al de la solución.
...
!*** Alfa es un coeficiente de calibración en la condición de goteo.
...
read(4,27)a	 
 ...
 enddo
close(4) 
!*** Sólo se dan valores iguales en la primera iteración
do u=1,i						 !*** Añadida esta línea
if (vb(2*i+u).eq.vitv(u))then	 !*** Añadida esta línea
zzp(u)=z(u)						 !*** Añadida esta línea
endif							 !*** Añadida esta línea
enddo							 !*** Añadida esta línea
...
if (modelo.eq.'superficial') then
 ...
 call mallaaguassomeras (i,j,z,zzp,vtv,velx,vely,eval,vo,vt)  	 !*** Añadido zzp
  !Aproximación inicial con las ecuaciones N-S 2D. Se puede usar siempre, cuando se ha seleccionado una resolución implícita o semi-implícita
  !diciéndose en el segundo caso el número de iteraciones temporales (ambas ecuaciones se resolverán con el mismo esquema como debe hacerse). 
  if (ap.eq.'si') then                                                                                              
  ...
  if (tol.ne.1e-5) then
   !No hecho todo el número previo de iteraciones NS si existe convergencia antes.
   !Reescrita la tolerancia tras iteraciones previas si hay convergencia. Así, se empieza por Sw después.
   tol=1e-5
   exit
   endif
  enddo                                                                                                                          
...
elseif (modelo.eq.'subterraneo') then 
 ...
 !Esquema implícito - Dentro de la subrutina aguassubterranea se hacen iteraciones para un incremento de tiempo o para el problema estacionario.
 call aguassubterranea(i,j,x,y,z,zp,zzp,nd,kix,kiy,ag,sino,bcg,At,vn,vov,eval,vb,ql,qb,qx,qy,vibv,vtv,velx,vely)  !*** Añadidos z,zzp,vb
 if (sino.eq.'si')then
 write(6,*)'At=',Ata+At
 endif
...
elseif (modelo.eq.'conjunto') then   
 ...
 !Inicio de iteraciones conjuntas: 
  ...
  !subterráneo hasta convergencia (Picard) y se tiene la solución conjunta para este incremento de tiempo, o la solución conjunta estacionaria.	       
   call aguassubterranea(i,j,x,y,z,zp,zzp,nd,kix,kiy,ag,sino,bcg,At,vn,vov,eval,vb,ql,qb,qx,qy,vibv,vtv,velx,vely)	   !*** Añadidos z,zp,vb
  ...
  call mallaaguassomeras (i,j,z,zzp,vtv,velx,vely,eval,vo,vt)		 !*** Añadido zzp
   ...
   !Aproximación inicial con las ecuaciones N-S 2D (ambas ecuaciones superficiales se resolverán implícitamente).
   ...
   if (tol.ne.1e-6) then
	!En este caso es necesario reescribir la tolerancia para que se evalúe bien la convergencia del sistema no lineal de aguas someras y evitar 
	!que se obligue a hacer una iteración conjunta solo
    tol=1e-6
    exit
    endif
   enddo                                                                                                                          
  ...
  !O bien toda la solución previa es superficial. 
  ...
  !iteración conjunta.
  !O bien se ha alcanzado convergencia para el modelo superficial (mínimo de 2 iteraciones)
   do u=1,i
   eval(u)=sqrt(3.0_8)
   enddo
  else
  !La solucion previa es subterránea o superficial y subterránea
  ...
  !(que irán en vov(i)). Aquí también se selecciona la malla para flujo subterráneo, definida por el dominio superficial (la que malla que queda).   						 
   ...
   !Se hace un mínimo de 2 iteraciones conjuntas con Sw por paso de tiempo. Si no se hiciese esto se hará 1 iteración si la diferencia es mínima 
   !en la solución superficial para el siguiente paso de tiempo lo cual es improbable (o tras la resolución con 2D NS) y no se tendría la solución
   !subterránea. Tampoco se tendría un buen procedimiento si se calculase. No se consideraría la solución subterránea obtenida en ese paso de 
...
read(3,26)no(1),no(4),no(2),no(5),no(3),no(6) 
 ...
 if (zp(no(ui)).eq.zzp(no(ui)))then	                                !*** Añadida esta línea
 ht(no(ui))=vtv(no(ui))
 endif                                                              !*** Añadida esta línea
 if ((zp(no(ui)).eq.zzp(no(ui))).or.(zp(no(b)).eq.zzp(no(b)))) then !*** Añadida esta línea
...
elseif (modelo.eq.'subterraneo') then
 ...
 vib(i+u)=vy(u)
 !*** Primera opción
 if (zp(u).ne.zzp(u))then	!*** Añadida esta línea
 vib(2*i+u)=vt(2*i+u)		!*** Añadida esta línea
 vibv(u)=vtv(u)				!*** Añadida esta línea
 else						!*** Añadida esta línea
 vib(2*i+u)=ht(u)
 vibv(u)=ht(u)
 endif					    !*** Añadida esta línea
 !*** Otra opción (parece mejor dd no hay sol temporal de un modelo se da el terreno)
 !*** vib(2*i+u)=vt(2*i+u)
...
if (((it.lt.5).and.(newton.eq.'si')).or.(newton.eq.'no')) then
 ...
 enddo
  ...
  if (bcg.eq.'no') then
	...
	!Se calcula yendo de C en C iteraciones con PCGB y haciéndolo hasta 3 veces.
	!Se para el programa al llegar a tres veces y se le pide que esté atento a si hay problemas 
	!a partir de ahí y que escoja otro método si es así.
     if (imp.eq.'si') then
...
!---------------------------------------------------------------------------------------------------------------------------------------------------------
subroutine aguassubterranea(i,j,x,y,z,zp,zzp,nd,kix,kiy,ag,sino,bcg,At,vnin,vov,eval,vb,ql,qb,qx,qy,vibv,vtv,velx,vely) 	 !*** Añadidos z,zzp,vb
use allocatacion
...
real*8 x(i),y(i),z(i),zp(i),zzp(i),tol,eval(i),At,vov(i),ag(i),alfa,vb(3*i)             !*** Añadidos z(i),zzp(i),alfa,vb(3*i)
...
vbv(u)=0.0_8
enddo
!*** Al subir alfa pasa mucho más caudal en la interfaz y tarda más iteraciones (probado de 0.00005 a 0.0005 - entre 0.0001 y 0.0005 hay mucho cambio y 
!*** para 0.001 daría error). En general alfa debe ser mayor que la conductividad mínima y tanto mayor sea se debe trabajar con At menores al variar mucho 
!*** más la solucion.
!*** Para un gran paso de caudal en el embalse la lámina toma curvatura para los números de manning existentes
alfa=0.000025 !*** Añadida esta línea
...
call vectorcontornocaudales(i,j,x,y,zp,zzp,veic,qxx,qyy,vb,alfa)    !*** Añadidos zp,zzp,vb,alfa
!Cálculo de las integrales correspondientes a la lluvia.
...
vbdin(u)=vbv(u)
enddo
!Aquí sólo se usa el esquema implícito.
conv=1
u=0 
!Improbable que haya algún problema (conv será 0 tras llamar a cualquiera de las opciones para resolver el sistema)
if (bcg.eq.'no') then
 do while (conv.eq.1)
 call gradientesbiconjugados(vecdin,c,vbdin,nonzero,conv)
 nonzero=.true.
...
!*** Conservación de masa en el contorno móvil
call conservacion (i,j,x,y,z,zp,zzp,vtv,velx,vely,vb) !*** Añadida esta línea
...
!----------------------------------------------------------------------------------------------------------------------------------------------
subroutine mallaaguassomeras(i,j,z,zzp,vta,velx,vely,eval,vuc,vt)  !*** Añadido zzp
use interaccion
integer*4 i,j,u,ui,uuu,uu,io		   
real*8 z(i),zzp(i),vt(3*i),vta(i),velx(i),vely(i),eval(i),vuc(3*i),fi	 !*** Añadido zzp(i)											   
character fe*1
 ...
 read(4,26)no(1),no(4),no(2),no(5),no(3),no(6)
   ...
   elseif ((vta(no(ui)).ne.0.0_8).and.(vta(no(b)).ne.0.0_8).and.(vta(no(c)).eq.0.0)) then
    ...
    enddo
	 ...
	 !Si se tienen dos elementos no particulares cuyo lado compartido tiene nodos lineales que se apoyan en contorno móvil pero el cuadrático
	 !no lo hace, se darían condiciones de contorno con ambos elementos en dicho nodo. No se deben dar y aquí se elimina la condición de contorno 
	 !la segunda vez que se vaya a dar. Si hay condición de contorno se elimina. Ningún elemento particular que la elimine será considerado después.
...
read(4,23)a
 ...
 read(4,26) no(1),no(4),no(2),no(5),no(3),no(6)   
   ...
   if ((z(no(1)).eq.zzp(no(1))).and.(z(no(2)).eq.zzp(no(2))).and.(z(no(3)).eq.zzp(no(3)))) then	  !*** Cambiados los dos if que había por este if
   uu=uu+1
   write(2,27)uu,no(1),no(4),no(2),no(5),no(3),no(6)		
   do uuu=1,6
   eval(no(uuu))=0.0_8
   enddo 
    ...
    e=ui+4-sb(ui)
    f=ui+5-sb(ui+1) 
	 !Elemento no pegado a la última selección (pegados por 2 nodos o 1 nodo sin lado pegado o totalmente separados).
   ...
   !*** Eliminado uno de los endif que había
 enddo
...
!--------------------------------------------------------
 ...
 read(7,26)no(1),no(4),no(2),no(5),no(3),no(6)
   ...
   !este tipo compartirán este nodo de calado positivo, y este nodo de la malla formará finalmente parte de un elemento de los analizados aquí. 
	...
	!*** Se han eliminado dos líneas que había aquí   		  	 
	...
	!*** zzp sólo se usa para seleccionar el contorno móvil
	zzp(no(ui))=z(no(ui))
	zzp(no(b))=z(no(b))	      
   endif
   enddo   
...
!La integración se ha hecho analíticamente (a mano) obteniendo las expresiones necesarias.
!--------------------------------------------------------------------------------------------------
subroutine vectorcontornocaudales(i,j,x,y,zp,zzp,vector,qxx,qyy,vb,al)         !*** Añadidos zp,zzp,vb,al
use elemental								                                   !*** Añadida esta línea
integer*4, dimension(:),allocatable::po                                        !*** Añadida esta línea
integer*4 i,j,sb(4),a,b,c										               !*** Eliminados n(6),u,ui
real*8 vector(i),x(i),y(i),ax,ay,qxx(i),qyy(i),zp(i),zzp(i),al,vb(3*i),Nn(3,3) !*** Eliminados xa,ya y añadidos zp(i),zzp(i),al,vb(3*i),Nn(3,3)
																               !*** Eliminado character ac*80

allocate(po(i))    !*** Añadida esta línea 
...
read(3,40)n(1),n(4),n(2),n(5),n(3),n(6) 
 do uu=1,3		   !*** Añadida esta línea
 do uuu=1,3		   !*** Añadida esta línea
 Nn(uu,uuu)=0.0_8  !*** Añadida esta línea
 enddo			   !*** Añadida esta línea
 enddo			   !*** Añadida esta línea
 do ui=1,3
   a=ui
   ...
   if ((zp(n(c)).eq.zzp(n(c))).or.(zp(n(b)).eq.zzp(n(b)))) then !*** Añadida esta línea
   ...
   !*** Integrales de contorno diferentes para condición de goteo																		 
   ...
   vector(n(c))=vector(n(c))-al*(2.0_8*vb(2*i+n(c))+vb(2*i+n(b)))/6.0_8																 !*** Añadida esta línea
   vector(n(b))=vector(n(b))-al*(2.0_8*vb(2*i+n(b))+vb(2*i+n(c)))/6.0_8  															 !*** Añadida esta línea
   Nn(c,c)=al*2.0_8/6.0_8																											 !*** Añadida esta línea
   Nn(c,b)=al*1.0_8/6.0_8																											 !*** Añadida esta línea
   Nn(b,c)=al*1.0_8/6.0_8																											 !*** Añadida esta línea
   Nn(b,b)=al*2.0_8/6.0_8																											 !*** Añadida esta línea
   endif																															 !*** Añadida esta línea
   !*** Cambio el comentario de dos líneas que había por el de debajo
   !*** En el contorno móvil las integrales de contorno se calculan con caudales que no dependen de velx y vely sino de la diferencia 
   !*** de altura entre flujo superficial y subterráneo (la velocidad es implícita en la fórmula). Estos valores calculados se tienen 
   !*** en cuenta al no imponer condiciones de nivel sino de caudal en el contorno móvil.
 enddo
 !*** Necesario hacer sumas para cada elemento para rellenar todos los espacios en su posición.
 !*** Sólo habrá un lado que genera coeficientes (si hubiese más el elemento pertenece a As) y por tanto no hace falta hacer sumas en Nn. Se 
 !*** podría poner un exit arriba.
 call suma(Nn,n,0,0,i,3,3,po)	!*** Añadida esta línea
 enddo
close(3)
deallocate(po)	!*** Añadida esta línea
end
...
!*** ---------------------------------------------------------------------------------------------------------------------------       
!*** Cálculo postproceso de velocidades que generan la conservación de masa entre ambos flujos									       
!*** ---------------------------------------------------------------------------------------------------------------------------       
subroutine conservacion (i,j,x,y,z,zp,zzp,vv,velx,vely,vb)																	        !*** Añadida esta línea
use allocatacion																											        !*** Añadida esta línea
integer*4, dimension(:),allocatable::po,Esb,Ese																				        !*** Añadida esta línea
integer*4 i,j,u,uu,uuu,ui,w,cc,ndim,Et,a,b,c,sb(4),n(6),conv 																	        !*** Añadida esta línea
real*8, dimension(:),allocatable::es,esp,vec,vecdin,vbdin,vic																        !*** Añadida esta línea
real*8 x(i),y(i),xa,ya,z(i),zp(i),zzp(i),vv(i),velx(i),vely(i),vb(3*i),Nn(3,3),Nm(3,3),ax,ay								        !*** Añadida esta línea
character ac*80																												        !*** Añadida esta línea
logical nonzero		 																										        !*** Añadida esta línea
																															        !*** Añadida esta línea
allocate(es(i),esp(i),po(2*i),vec(2*i),Esb(2*i),Ese(2*i),vic(2*i))															        !*** Añadida esta línea
																															        !*** Añadida esta línea
 39     format(4/,A80)																										        !*** Añadida esta línea
 40     format(6X,6(X,I5))																									        !*** Añadida esta línea
																															        !*** Añadida esta línea
!*** Inizialización previa de variables:																						       
!*** -----------------------------------																						       
sb=(/0,0,3,3/)																												        !*** Añadida esta línea
Et=0																														        !*** Añadida esta línea
do u=1,2*i																													        !*** Añadida esta línea
Esb(u)=0																													        !*** Añadida esta línea
Ese(u)=0																													        !*** Añadida esta línea
vec(u)=0.0_8																												        !*** Añadida esta línea
vic(u)=0.0_8																												        !*** Añadida esta línea
enddo																														        !*** Añadida esta línea
do u=1,i																													        !*** Añadida esta línea
es(u)=vv(u)-zp(u)																											        !*** Añadida esta línea
esp(u)=vb(2*i+u)-z(u)																											    !*** Añadida esta línea
enddo																															    !*** Añadida esta línea
 																																    !*** Añadida esta línea
!*** Realmente es necesario resolver un sistema para garantizar la conservación.
!*** En el término independiente el cálculo de las integrales para tres puntos con velx y espesor subt y en la matriz del sistema 
!*** irá el calado H siendo la velocidad superficial resuelta en todos los nodos del contorno.
																																	!*** Añadida esta línea
!*** Cálculo del término independiente y de valores de las variables para el cálculo de las dimensiones
!*** --------------------------------------------------------------------------------------------------
!*** Sólo se considera el contorno móvil 
open(unit=3,file='C:\mallasub.txt',status='old')																				    !*** Añadida esta línea
read(3,39)ac																													    !*** Añadida esta línea
do u=1,j																														    !*** Añadida esta línea
read(3,40)n(1),n(4),n(2),n(5),n(3),n(6)																							    !*** Añadida esta línea
 do ui=1,3																														    !*** Añadida esta línea
  a=ui																															    !*** Añadida esta línea
  b=ui+1-sb(ui)																													    !*** Añadida esta línea
  c=ui+2-sb(ui+1)																												    !*** Añadida esta línea
  if ((zp(n(c)).ne.zzp(n(c))).and.(zp(n(b)).ne.zzp(n(b)))) then																	    !*** Añadida esta línea
  xa=x(n(c))-x(n(b)) 																											    !*** Añadida esta línea
  ya=y(n(c))-y(n(b)) 	 																										    !*** Añadida esta línea
  Esb(n(a))=Esb(n(a))+3																											    !*** Añadida esta línea
  Esb(n(c))=Esb(n(c))+3																											    !*** Añadida esta línea
  Esb(n(b))=Esb(n(b))+3																											    !*** Añadida esta línea
  Ese(n(c))=1																													    !*** Añadida esta línea
  Ese(n(b))=1   																												    !*** Añadida esta línea
  call direccion(x,y,i,n(b),n(c),n(a),ax,ay)  																					    !*** Añadida esta línea
  vec(n(c))=vec(n(c))+(3.0_8*velx(n(c))*es(n(c))+velx(n(b))*es(n(c))+velx(n(c))*es(n(b))+velx(n(b))*es(n(b)))*ax*abs(ya)/12.0_8	    !*** Añadida esta línea
  vec(n(b))=vec(n(b))+(velx(n(c))*es(n(c))+velx(n(b))*es(n(c))+velx(n(c))*es(n(b))+3.0_8*velx(n(b))*es(n(b)))*ax*abs(ya)/12.0_8     !*** Añadida esta línea
  vec(i+n(c))=vec(i+n(c))+(3.0_8*vely(n(c))*es(n(c))+vely(n(b))*es(n(c))+vely(n(c))*es(n(b))+vely(n(b))*es(n(b)))*ay*abs(xa)/12.0_8	!*** Añadida esta línea
  vec(i+n(b))=vec(i+n(b))+(vely(n(c))*es(n(c))+vely(n(b))*es(n(c))+vely(n(c))*es(n(b))+3.0_8*vely(n(b))*es(n(b)))*ay*abs(xa)/12.0_8 !*** Añadida esta línea
  endif																																!*** Añadida esta línea
 enddo																																!*** Añadida esta línea
enddo																																!*** Añadida esta línea
do u=1,i																															!*** Añadida esta línea
Esb(i+u)=Esb(u)																														!*** Añadida esta línea
Ese(i+u)=Ese(u)																														!*** Añadida esta línea
enddo																																!*** Añadida esta línea
 																																	!*** Añadida esta línea
!*** Cálculo de las dimensiones
!*** --------------------------
do u=1,2*i																															!*** Añadida esta línea
Et=Et+Esb(u)																														!*** Añadida esta línea
enddo																																!*** Añadida esta línea
Et=Et+2*i+1																															!*** Añadida esta línea
allocate (ita(Et),sa(Et))																											!*** Añadida esta línea
do u=1,Et																															!*** Añadida esta línea
ita(u)=0																															!*** Añadida esta línea
sa(u)=0.0_8																															!*** Añadida esta línea
enddo																																!*** Añadida esta línea
ita(1)=2*i+2	   																													!*** Añadida esta línea
do u=1,2*i																															!*** Añadida esta línea
ita(u+1)=ita(u)+Esb(u)																												!*** Añadida esta línea
enddo																																!*** Añadida esta línea
ndim=ita(2*i+1)-1																													!*** Añadida esta línea
do u=1,2*i																															!*** Añadida esta línea
po(u)=ita(u)																														!*** Añadida esta línea
enddo																																!*** Añadida esta línea
 																																	!*** Añadida esta línea
!*** Cálculo de los valores de las matrices para cada elemento 
!*** ---------------------------------------------------------
!*** Sólo se considera el contorno móvil
rewind(3)																															!*** Añadida esta línea
 read(3,39)ac																														!*** Añadida esta línea
 do u=1,j																															!*** Añadida esta línea
 read(3,40)n(1),n(4),n(2),n(5),n(3),n(6)																							!*** Añadida esta línea
 do uu=1,3																															!*** Añadida esta línea
 do uuu=1,3																															!*** Añadida esta línea
 Nn(uu,uuu)=0.0_8																													!*** Añadida esta línea
 Nm(uu,uuu)=0.0_8																													!*** Añadida esta línea
 enddo																																!*** Añadida esta línea
 enddo																																!*** Añadida esta línea
  do ui=1,3																															!*** Añadida esta línea
   a=ui																																!*** Añadida esta línea
   ...
   ya=y(n(c))-y(n(b)) 																												!*** Añadida esta línea
   call direccion(x,y,i,n(b),n(c),n(a),ax,ay)	 																					!*** Añadida esta línea
   Nn(c,c)=(3.0_8*esp(n(c))+esp(n(b)))*ax*abs(ya)/12.0_8																			!*** Añadida esta línea
   Nn(c,b)=(esp(n(c))+esp(n(b)))*ax*abs(ya)/12.0_8																					!*** Añadida esta línea
   Nn(b,c)=(esp(n(c))+esp(n(b)))*ax*abs(ya)/12.0_8																					!*** Añadida esta línea
   Nn(b,b)=(esp(n(c))+3.0_8*esp(n(b)))*ax*abs(ya)/12.0_8   																			!*** Añadida esta línea
   Nm(c,c)=(3.0_8*esp(n(c))+esp(n(b)))*ay*abs(xa)/12.0_8																			!*** Añadida esta línea
   Nm(c,b)=(esp(n(c))+esp(n(b)))*ay*abs(xa)/12.0_8																					!*** Añadida esta línea
   Nm(b,c)=(esp(n(c))+esp(n(b)))*ay*abs(xa)/12.0_8																					!*** Añadida esta línea
   Nm(b,b)=(esp(n(c))+3.0_8*esp(n(b)))*ay*abs(xa)/12.0_8 																			!*** Añadida esta línea
   !*** Realmente sólo habrá un lado que genera coeficientes (si hubiese más el elemento pertenece a As) y se puede poner un exit
   !*** Si hubiese más está programado para tenerlo en cuenta.
   call suma(Nn,n,0,0,2*i,3,3,po)																									!*** Añadida esta línea
   call suma(Nm,n,i,i,2*i,3,3,po)																									!*** Añadida esta línea
   endif																															!*** Añadida esta línea
 enddo																																!*** Añadida esta línea
enddo																																!*** Añadida esta línea
																																	!*** Añadida esta línea
call orden (2*i,w)																													!*** Añadida esta línea
ndim=ndim-w																															!*** Añadida esta línea
																																	!*** Añadida esta línea
!*** Cálculo de la velocidad en contornos paralelos a x o a y
!*** --------------------------------------------------------
!*** Con el sistema cuido de que la aportación sobre el vector normal de cada componente sea la misma. Sin embargo si hay contornos 
!*** paralelos a x o y una de las componentes de la velocidad no aporta valor al vector normal, y por tanto quedaría indeterminada de 
!*** cara a la continuidad.  
!*** P.e. si tengo contorno paralelo a x queda indeterminada la velocidad en dirección x, podría ser cualquiera que no aportará caudal y 
!*** no intervendrá en la continuidad. En este caso se tendría una fila de ceros en la matriz y el vector (ax nulo) y doy una velocidad 
!*** promediada considerado el cambio de espesor.  
do u=1,2*i																														    !*** Añadida esta línea
if ((vec(u).eq.0.0_8).and.(Ese(u).ne.0))then																						!*** Añadida esta línea
Ese(u)=0																															!*** Añadida esta línea
  if (u.le.i)then																													!*** Añadida esta línea
  velx(u)=velx(u)*es(u)/esp(u)																										!*** Añadida esta línea
  else																																!*** Añadida esta línea
  vely(u-i)=vely(u-i)*es(u-i)/esp(u-i)  																									!*** Añadida esta línea
  endif																																!*** Añadida esta línea
endif																																!*** Añadida esta línea
enddo																																!*** Añadida esta línea
																																	!*** Añadida esta línea
...
!*** ----------------------
cc=0																																!*** Añadida esta línea
do u=1,2*i																															!*** Añadida esta línea
 if (Ese(u).ne.0) then 																												!*** Añadida esta línea
 !*** También se eliminan las filas y columnas relativas a los nodos donde el contorno es paralelo a x o a y.
 cc=cc+1																															!*** Añadida esta línea
 vec(cc)=vec(u) 																													!*** Añadida esta línea
 vic(u)=sqrt(2.0_8)																													!*** Añadida esta línea
 endif 																																!*** Añadida esta línea
enddo																																!*** Añadida esta línea
 																																	!*** Añadida esta línea
call reduccion(ndim,vic,2*i)																										!*** Añadida esta línea
nonzero=.false.																														!*** Añadida esta línea
allocate(vbdin(cc),vecdin(cc))  																									!*** Añadida esta línea
do u=1,cc																															!*** Añadida esta línea
vecdin(u)=vec(u)																													!*** Añadida esta línea
vbdin(u)=0.0_8																														!*** Añadida esta línea
enddo	
conv=1																															!*** Añadida esta línea
call gradientesbiconjugados(vecdin,cc,vbdin,nonzero,conv)																				!*** Añadida esta línea
																																	!*** Añadida esta línea
w=0																																	!*** Añadida esta línea
do u=1,2*i																															!*** Añadida esta línea
  if (vic(u).ne.0.0_8) then																											!*** Añadida esta línea
  w=w+1																																!*** Añadida esta línea
  if (u.le.i)then																													!*** Añadida esta línea
  velx(u)=vbdin(w)																													!*** Añadida esta línea
  else																																!*** Añadida esta línea
  vely(u-i)=vbdin(w)																													!*** Añadida esta línea
  endif																																!*** Añadida esta línea
  endif																																!*** Añadida esta línea
enddo																																!*** Añadida esta línea
 close(3)																															!*** Añadida esta línea
 write(6,*)'Solucion de velocidades conservativas'																					!*** Añadida esta línea
 deallocate(es,esp,po,vec,Esb,Ese,vecdin,vbdin,vic,ita,sa)																			!*** Añadida esta línea
end																																	!*** Añadida esta línea
																																	!*** Añadida esta línea
!---------------------------------------------------------------------------------------------------------------------------------------
...
!Es irrelevante el valor que se defina aquí dado que se estimará el error antes de la primera iteración en la función isdbcg y se usará 
!ese error. En gradientesbiconjugados no se estimará antes de la primera iteración y se considera que este error es 1.
er=1.0_8
ie=0
conv=0
